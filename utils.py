...
class Agent():
    def __init__(self, config: dict):
        ...
        # Cache the stock's long name to avoid slow yf.Ticker(symbol).info calls inside the loop.
        print(f"Fetching stock info for {config['stock_symbol']}...")
        try:
            stock = yf.Ticker(config['stock_symbol'])
            self.stock_name = stock.info.get('longName', config['stock_symbol'])
        except Exception:
            self.stock_name = config['stock_symbol']
        print(f"Stock name resolved: {self.stock_name}")
